...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # """
        # NOTE the usage of S-1/S/S+1 is messed up and confusing here
        bsz, full_seqlen, _ = x.shape  # full_seqlen includes real embeds and pred embeds
        # this is just the condition plus all original tokens, +1 is bc of condition
        ...
        # use 2 since are the next preds and also have time embeddings and thus need to condition on two tokens
        xq_p, xk_p = apply_rotary_emb(xq_p, xk_p, freqs_cis=freqs_cis[2:original_seqlen+1])
        ...
        # B, N, S-1, S; this uses xq_p and keys_o since for every next pred calcs similarity to all prev words; right S is because have extra condition
        scores_p = torch.matmul(xq_p, keys_o.transpose(2, 3)) / math.sqrt(self.head_dim)

        # B, N, S-1, 1; is used since context_length = original_length +1, superdiag needs this
        ...
        # is B, N, S-1, S; represents for each next pred (S-1 row) attending to all previous words (S-1) and then itself +1
        ...
        # [2, ..., S] (len S-1); added 1 to superdiag from default_ebt since have extra time condition
        superdiag_cols = torch.arange(2, scores_p.shape[3])
        ...
        # for zeroing out superdiag since dont want to include in matmul, do this in differentiable way
        ...
        # is B, N, S; basically how much each token on this superdiag should attent to itself; clone since dont want mask to change this
        scores_p_superdiagonal = scores_p.diagonal(offset=2, dim1=2, dim2=3).clone()

        # keeps scores_p as is except for superdiagonal which is next preds attention to selves, cant multiply these naively by values_p or values_o
        scores_p = scores_p * diagonal_removal_mask

        # B, N, S-1, S now; next preds/scores_p_superdiagonal was why needed extra col earlier (temp_append)
        scores_p = scores_p[:, :, :, :-1]
        # B, N, S-1, H; is how next preds attend to all original previous tokens;
        ...
        # B, N, S-1, H this is for weighted sum of each next pred to its final embed rep.
        next_pred_self_attention = values_p * scores_p_superdiagonal.unsqueeze(dim=-1)

        # B, N, S-1, H adding this is adding the aspect of each next pred embedding attending to itself
...
class EBTTimeConcat(nn.Module):
    ...
    def forward(self, embeddings: torch.Tensor, start_pos: int, mcmc_step=0):
        ...
        # do this since passed in seqlen is 2(S-1)+1 so add 3 div 2 = S+1 which corresponds to concatting time embed
        seqlen = (seqlen+3) // 2
