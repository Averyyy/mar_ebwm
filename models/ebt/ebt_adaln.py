...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # B, N, S-1, S-1; this uses xq_p and keys_o since for every next pred calcs similarity to all prev words
        ...
        # is B, N, S-1; basically how much each token on this superdiag should attent to itself; clone since dont want mask to change this
        ...
        # B, N, S-1, S-1 now; next preds/scores_p_superdiagonal was why needed extra col earlier (temp_append)
        scores_p = scores_p[:, :, :, :-1]
...
class EBTAdaLN(nn.Module):
    ...
    def forward(self, embeddings: torch.Tensor, start_pos: int, mcmc_step=None, c=None):
        ...
        if c is not None and mcmc_time_emb is not None:
            ...
            time_embeddings = c
        elif mcmc_time_emb is not None:
            time_embeddings = mcmc_time_emb
        else:
            raise AssertionError("Either `c` or `mcmc_step` must be provided for time embeddings.")
